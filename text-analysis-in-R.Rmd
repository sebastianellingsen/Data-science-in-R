...
Want it as a one-token-per-document-per-row format:
...
`tidy_books` now gives a tidy dataset of the books. The next step is to remove stop words:
...
We need the data as a document term matrisx in order to use learning algorithms.
```{r, echo=TRUE, results='hide'}
